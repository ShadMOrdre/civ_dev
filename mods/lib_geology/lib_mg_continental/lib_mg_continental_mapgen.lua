...
local C = lib_mg_continental.C
	...
	local mg_type = lib_mg_continental.mg_type
	local mg_3d = lib_mg_continental.mg_3d
	...
	local np_3dterrain		= P["np_3dterrain"]
	...
	minetest.register_on_generated(function(minp, maxp, seed)
		...
		local permapdims3d_2 = {x = sidelen, y = sidelen, z = sidelen}
		...
		if mg_3d == true then
			nobj_3dterrain = nobj_3dterrain or minetest.get_perlin_map(np_3dterrain, permapdims3d_2)
			nobj_3dterrain:get3dMap_flat(minp, isln_3dterrain)
		end
	...
	--2D HEIGHTMAP GENERATION
		...
		for z = minp.z, maxp.z do
			for x = minp.x, maxp.x do
				...
				--local pos_2d = {x = x, z = z}

				lib_mg_continental.cellmap[index2d] = {}

				if mg_type == "voronoi" then
					...
					lib_mg_continental.cellmap[index2d].m = m_idx
					lib_mg_continental.cellmap[index2d].p = p_idx
					lib_mg_continental.cellmap[index2d].c = c_idx
					...
					local vterrain = (base_rng - (vcontinental * 0.5))
					...
					--local t_c = 0
					--t_y, t_c = lib_mg_continental.get_terrain_height_cliffs(velevation,ncliff)
					--lib_mg_continental.cliffmap[index2d] = t_c

					t_y = velevation

				end

				if mg_type == "voronoi_cliffs" then
					local c_idx, c_dist, c_rise, c_run, c_edge = lib_mg_continental.get_nearest_cell({x = x, z = z}, dist_metric, 3)
					local p_idx, p_dist, p_rise, p_run, p_edge = lib_mg_continental.get_nearest_cell({x = x, z = z}, dist_metric, 2)
					local m_idx, m_dist, m_rise, m_run, m_edge = lib_mg_continental.get_nearest_cell({x = x, z = z}, dist_metric, 1)
				...
				if mg_type == "noise" then 
					...
					--local t_c = 0
					--t_y, t_c = lib_mg_continental.get_terrain_height_cliffs((nelevation),ncliff)
					--lib_mg_continental.cliffmap[index2d] =  t_c
				...
				if mg_type == "all" then
					...
					local vterrain = (base_rng - (vcontinental * 0.5))
					local vheight = ((base_max - vcontinental) * mg_golden_ratio)
					--local vheight = ((base_max - vcontinental) * 0.0125)

					local nterrain = isln_terrain[z-minp.z+1][x-minp.x+1]
					local nheight = nterrain * mg_golden_ratio
					--local nheight = nterrain * 0.0125
					local ncliff = isln_cliffs[z-minp.z+1][x-minp.x+1]
					...
					local t_c = 0
					t_y, t_c = lib_mg_continental.get_terrain_height_cliffs((velevation + nelevation),ncliff)
					lib_mg_continental.cliffmap[index2d] = t_c
		...
		for z = minp.z, maxp.z do
			for y = minp.y, maxp.y do
				for x = minp.x, maxp.x do
					...
					local t_cliff = lib_mg_continental.cliffmap[index2d] or 0
	...
	--
					if mg_3d == true then
						local density_noise = isln_3dterrain[ni]
						local density_gradient = (1 - y) / base_rng
	...
	--
					if y <= theight then
						...
						local t_heat_mid = ((lib_mg_continental.mg_map_size * lib_mg_continental.mg_world_scale) * 0.25)
						...
						local t_map_scale = t_heat_factor / lib_mg_continental.mg_world_scale
						t_heat = t_diff * t_map_scale

						local nheat = (nbase_heatmap[index2d] + nbase_heatblend[index2d]) + t_heat
						...
						--	z=z,
						--}
						lib_mg_continental.heatmap[index2d] = nheat
						lib_mg_continental.humiditymap[index2d] = nhumid
	...
	--
					...
					if mg_3d == true then
						...
						if (theight > -1) then
							if (density_height > 0) then
								if lib_mg_continental.biomemap[index2d]:find("_arid") then
									t_node = t_stone
								end
								if lib_mg_continental.biomemap[index2d]:find("_mountain") then
									t_node = t_stone
								end
							end
						end
